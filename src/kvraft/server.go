...
type KVServer struct {
	...
	waitCommitCh    map[int]*chan Result // 映射 startIndex->ch
...
func (kv *KVServer) HandleOp(opArgs *Op) (res Result) {
	...
	kv.waitCommitCh[startIndex] = &newCh
	...
	defer func() {
		kv.mu.Lock()
		delete(kv.waitCommitCh, startIndex)
		close(newCh)
		kv.mu.Unlock()
	}()
...
func (kv *KVServer) ApplyHandler() {
	for !kv.killed() {
		...
		if log.CommandValid {
			...
			ch, exist := kv.waitCommitCh[log.CommandIndex]
			if exist {
				kv.mu.Unlock()
				// 发送消息
				func() {
...
func StartKVServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int) *KVServer {
	...
	kv.waitCommitCh = make(map[int]*chan Result)
