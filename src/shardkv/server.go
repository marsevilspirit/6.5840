...
type Op struct {
	...
	NewConfig   shardctrler.Config // 新版本的config（仅UpdateConfig指令有效）
...
type ShardKV struct {
	...
	mck                   *shardctrler.Clerk              // 关联shardmaster
	...
	ownedShards           [shardctrler.NShards]ShardState // 该ShardKV负责的shard的状态
	preConfig             shardctrler.Config              // 该ShardKV当前的上一个config（用来找前任shard拥有者）
	curConfig             shardctrler.Config              // 该ShardKV目前所处的config
}
